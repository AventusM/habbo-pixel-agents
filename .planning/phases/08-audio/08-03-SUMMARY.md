...
tags: [webview, csp, audio-integration, user-gesture]

# Dependency graph
requires:
  - phase: 08-audio
    plan: 01
    provides: AudioManager class with init/loadSound/play methods
  - phase: 08-audio
    plan: 02
    provides: Audio asset pipeline (optional - silent fallback works without files)
  - phase: 06-font-bundling
    provides: asWebviewUri pattern for bundled assets
provides:
...
affects: [future audio features, webview security patterns]

# Tech tracking
tech-stack:
  added: [CSP media-src directive, webview audio URI generation]
  patterns: [Lazy audio initialization on user gesture, asWebviewUri for audio files]

key-files:
  created: []
  modified: [src/extension.ts, src/RoomCanvas.tsx]
...
requirements-completed: [AUDIO-05]
