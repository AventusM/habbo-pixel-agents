...
tags: [avatars, 8-directions, multi-layer, placeholder-sprites, webview-integration]
dependency_graph:
  requires: [01-01, 03-03, isometricMath, isoSpriteCache, depthSort]
  provides: [isoAvatarRenderer, avatar-placeholders, 8-direction-rendering]
  affects: [webview, RoomCanvas]
tech_stack:
  added: [ImageMagick, avatar-atlas-generation]
  patterns: [multi-layer-composition, graceful-degradation, 8-direction-sprites]
key_files:
  created:
    - src/isoAvatarRenderer.ts (120 lines)
    - tests/isoAvatarRenderer.test.ts (180 lines)
    - src/global.d.ts (25 lines)
    - scripts/generate-avatar-placeholders.sh (175 lines)
    - assets/spritesheets/avatar_atlas.png (99KB, 1536×1536px)
    - assets/spritesheets/avatar_atlas.json (48KB, 192 frames)
  modified:
    - tests/setup.ts (+8 lines, added window mock)
    - src/extension.ts (+13 lines, avatar atlas URIs)
    - src/webview.tsx (+13 lines, avatar atlas loading)
    - src/RoomCanvas.tsx (+52 lines, avatar rendering)
decisions:
  - Use 8 unique direction sprites instead of mirroring (avatars are asymmetric unlike furniture)
  - Skip clothing layer implementation in favor of simplified body/head/hair for placeholders
  - Render avatars in frame loop (not pre-rendered to offscreen canvas) to enable future animation
  - Use ImageMagick for placeholder generation with distinct color schemes per variant
metrics:
  duration: 5min
  tasks: 3
  files: 9
  ...
  completed: 2026-02-28T23:17:31Z
---
