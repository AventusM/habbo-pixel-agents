...
plan: "01"
subsystem: kanban-renderer
tags: [bugfix, kanban, sticky-notes, wall-rendering]
dependency_graph:
  requires: [12-03]
  provides: [single-wall-in-progress-constraint]
  affects: [src/isoKanbanRenderer.ts, tests/isoKanbanRenderer.test.ts]
tech_stack:
  added: []
  patterns: [left-wall-only constraint for In Progress sticky notes]
key_files:
  created: []
  modified:
...
decisions:
  - "In Progress notes confined to left wall only: right wall reserved exclusively for Done aggregate note"
  - "hasRightLarge retained as it guards Done note drawing; suppressed via void operator to avoid lint warning"
metrics:
  duration: "3min"
