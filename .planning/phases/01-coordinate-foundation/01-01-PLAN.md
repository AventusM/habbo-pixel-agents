...
must_haves:
  ...
  artifacts:
    ...
    - path: "tsconfig.json"
      provides: "TypeScript config for strict ESM compilation"
...
- `tsconfig.json` — strict TypeScript config targeting ESNext/ESM
...
Verify `node_modules/vitest` exists after install.
  ...
  <verify>
    <automated>cd /Users/antonmoroz/Documents/GitHub/habbo-pixel-agents && node -e "const pkg = JSON.parse(require('fs').readFileSync('package.json','utf8')); console.assert(pkg.type==='module','missing type:module'); console.assert(pkg.scripts.test,'missing test script'); console.log('package.json ok')" && node -e "const tc = JSON.parse(require('fs').readFileSync('tsconfig.json','utf8')); console.assert(tc.compilerOptions.strict,'missing strict'); console.log('tsconfig.json ok')" && ls node_modules/vitest > /dev/null && echo "vitest installed"</automated>
  </verify>
  <done>
    - `package.json` exists with `"type": "module"`, `"test": "vitest run"`, `"typecheck": "tsc --noEmit"` scripts
    - `tsconfig.json` exists with `"strict": true`, `"module": "ESNext"`, `"moduleResolution": "bundler"`
...
Run `npm run typecheck` — must exit 0 with no TypeScript errors.
