...
- D007 (M001/S01 planning): Use RetroDiffusion’s 48px character path as the first implementation target for the new avatar pipeline. -> Adopt the 48px `four_angle_walking_idle` / `8_dir_rotation` style set for S02, preserve the existing `pixellab` atlas + Texture-Packer `pl_*` manifest contract, and defer 104px PixelLab-parity composition to follow-up work.
